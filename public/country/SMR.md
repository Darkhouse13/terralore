# San Marino — national dossier

San Marino — economy, society, governance, health, education, technology, geography and resources in one sourced dossier: 35 indicators, each with its…

Canonical: https://terralore.co/country/SMR
Updated: 2026-08-14 (latest domain refresh; each figure carries its own observation year)

## Overview

- Official name: Most Serene Republic of San Marino
- Capital: City of San Marino
- Region: Southern Europe, Europe
- Population: 34,000
- Area: 61 km²
- Languages: Italian
- Currency: Euro (EUR)

A "—" is an absence in the source data, never a zero. Each figure shows the year it was observed — vintages differ by indicator and nation.

## Economy

| Metric | Value | Observed | Source |
|---|---|---|---|
| GDP | $2.0 B | 2023 | World Development Indicators |
| GDP per capita | $59,871 | 2023 | World Development Indicators |
| GDP growth | 0.4% | 2023 | World Development Indicators |
| Inflation | 2.3% | 2025 | World Development Indicators |
| Trade openness | 341.0% | 2023 | World Development Indicators |

## Society

| Metric | Value | Observed | Source |
|---|---|---|---|
| Population | 34,000 | 2025 | World Development Indicators |
| Life expectancy | 85.8 yrs | 2024 | World Development Indicators |
| Urban population | 97.2% | 2025 | World Development Indicators |
| Fertility rate | 1.2 | 2024 | World Development Indicators |
| Literacy rate | 99.9% | 2022 | World Development Indicators |

## Governance

| Metric | Value | Observed | Source |
|---|---|---|---|
| Voice & accountability | 78.7 | 2024 | Worldwide Governance Indicators (via World Bank API) |
| Political stability | 89.3 | 2024 | Worldwide Governance Indicators (via World Bank API) |
| Government effectiveness | 68.8 | 2024 | Worldwide Governance Indicators (via World Bank API) |
| Regulatory quality | 68.5 | 2024 | Worldwide Governance Indicators (via World Bank API) |
| Rule of law | 76.9 | 2024 | Worldwide Governance Indicators (via World Bank API) |
| Control of corruption | 71.7 | 2024 | Worldwide Governance Indicators (via World Bank API) |

## Health

| Metric | Value | Observed | Source |
|---|---|---|---|
| Health spending | 6.6% | 2023 | Global Health Observatory (WHO, via World Bank WDI) |
| Physicians | 4.63 | 2023 | Global Health Observatory (WHO, via World Bank WDI) |
| Under-5 mortality | 1.3 | 2024 | Child mortality & immunisation estimates (UN IGME and WHO/UNICEF, via World Bank WDI) |
| Measles immunisation | 85.0% | 2024 | Child mortality & immunisation estimates (UN IGME and WHO/UNICEF, via World Bank WDI) |
| Safe drinking water | 100.0% | 2024 | Global Health Observatory (WHO, via World Bank WDI) |

## Education

| Metric | Value | Observed | Source |
|---|---|---|---|
| Adult literacy | 99.9% | 2022 | Education statistics (UNESCO UIS, via World Bank WDI) |
| Education spending | 2.9% | 2022 | Education statistics (UNESCO UIS, via World Bank WDI) |
| Primary enrolment | 93.5% | 2024 | Education statistics (UNESCO UIS, via World Bank WDI) |
| Secondary enrolment | 60.4% | 2024 | Education statistics (UNESCO UIS, via World Bank WDI) |
| Tertiary enrolment | 57.5% | 2024 | Education statistics (UNESCO UIS, via World Bank WDI) |

## Technology

| Metric | Value | Observed | Source |
|---|---|---|---|
| Internet users | 97.4% | 2024 | ICT indicators (ITU, via World Bank WDI) |
| Mobile subscriptions | 120 | 2022 | ICT indicators (ITU, via World Bank WDI) |

## Geography

| Metric | Value | Observed | Source |
|---|---|---|---|
| Land area | 60 km² | 2023 | World Development Indicators |
| Forest area | 16.7% | 2023 | World Development Indicators |
| Agricultural land | 38.3% | 2023 | World Development Indicators |

## Resources

| Metric | Value | Observed | Source |
|---|---|---|---|
| Resource rents | 0.0% | 2020 | World Development Indicators |
| Mineral rents | 0.0% | 2020 | World Development Indicators |
| Forest rents | 0.0% | 2020 | World Development Indicators |
| Electricity access | 100.0% | 2024 | World Development Indicators |

Every metric above is also ranked across all nations: https://terralore.co/rankings (markdown twins at /rankings/<metric>.md).

Machine-readable claims — every figure above as an addressable claim ID (TL:SMR:<metric>:<year>) with value, source, license and a ready citation string: https://terralore.co/country/SMR.claims.json

History: "Liberty on a mountaintop: the world's oldest surviving republic, founded by a stonemason in 301." — the sourced chronicle is at https://terralore.co/country/SMR/chronicle.md (6 eras, 26 events).

## Sources

- World Development Indicators — World Bank · CC BY 4.0 · accessed 2026-08-14 · https://data.worldbank.org
- Worldwide Governance Indicators (via World Bank API) — Worldwide Governance Indicators / World Bank · CC BY 4.0 · accessed 2026-08-14 · https://www.worldbank.org/en/publication/worldwide-governance-indicators
- Global Health Observatory (WHO, via World Bank WDI) — WHO / World Bank · CC BY 4.0 · accessed 2026-08-14 · https://www.who.int/data/gho
- Child mortality & immunisation estimates (UN IGME and WHO/UNICEF, via World Bank WDI) — UNICEF and WHO / World Bank · CC BY 4.0 · accessed 2026-08-14 · https://childmortality.org
- Education statistics (UNESCO UIS, via World Bank WDI) — UNESCO Institute for Statistics / World Bank · CC BY 4.0 · accessed 2026-08-14 · https://uis.unesco.org
- ICT indicators (ITU, via World Bank WDI) — ITU / World Bank · CC BY 4.0 · accessed 2026-08-14 · https://www.itu.int/en/ITU-D/Statistics

Citation template: Terralore, "San Marino — national dossier", terralore.co/country/SMR, retrieved <YYYY-MM-DD>.
