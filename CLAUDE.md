...
- The landing page's staggered entrance is a **CSS animation** (`.reveal` /
  ...
  a JS-gated entrance.
- **The globe is deferred.** `public/globe-still.svg` (generated by
  `scripts/build-globe-still.mjs` from the same geojson + palette, framed at the
  globe's opening `pointOfView`) paints instantly; `AtlasHome` mounts `GlobeScene`
  on idle for capable desktops, on tap otherwise, and crossfades. `GlobeScene` is
  imported via `next/dynamic` **from AtlasHome** — a static import would pull
  three.js (~450 KB) back into the landing chunk even unrendered. Wheel zoom is
  off (`controls.enableZoom = false`): the home scrolls past the globe now, and a
  wheel-zooming canvas is a scroll trap.
